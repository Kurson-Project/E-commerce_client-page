...
const ProductPage = () => {
  ...
  useEffect(() => {
    const pathToCategoryMap: Record<string, string> = {
      ...
      "/products/ai-operating-systems": "AI Operating Systems",
    };
  ...
  useEffect(() => {
    const filtered = DataProduk.filter((produk) => {
      ...
      const matchCategory =
        selectedCategory === "All Products" || produk.category === selectedCategory;
        ...
        <div className="flex w-full md:flex-row flex-col justify-between items-center gap-2">
          <div className="flex gap-2 overflow-x-auto flex-nowrap w-full scrollbar-hidden py-4">
            <Button
              ...
              onClick={() => setTimeout(() => setSelectedCategory("All Products"), 1000)}
              asChild
            >
            ...
            {categories.map((category) => (
              <Button
                ...
                className="rounded-full shrink-0"
                onClick={() => setTimeout(() => setSelectedCategory(category.name), 1000)}
                asChild
              >
                        ...
                        {allTools.map((tool) => (
                          <CommandItem
                            ...
                            onSelect={() => {
                              if (filterTools.includes(tool)) {
                                setFilterTools(filterTools.filter(t => t !== tool));
                              } else {
                          ...
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                filterTools.includes(tool) ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {tool}
                          </CommandItem>
